...
def initialize_master (cl_args, num_workers):   
    ...
    configs ['worker_load']        = int (math.ceil (float(configs['sampling_rounds']) / float(num_workers))) 
    configs ['num_workers']        = num_workers
    
    super_init.save_network_stats         (M, configs)
    super_init.save_simulation_parameters (configs)  
    return M, configs
#--------------------------------------------------------------------------------------------------    
...
def load_simulation_configs (param_file, rank):
    ...
    configs['output_directory']    = util.slash(util.slash (util.slash (configs['output_directory']) +configs['stamp']))
    configs['stats_dir']           = configs['output_directory']+"00_network_stats/" 
    configs['datapoints_dir']      = configs['output_directory']+"03_execution_stats_simulation/data_points/"
    configs['params_save_dir']     = configs['output_directory']+"03_execution_stats_simulation/parameters/"
    configs['progress_file']       = configs['output_directory']+"03_execution_stats_simulation/progress.dat"
    configs['progress_dir']        = configs['output_directory']+"03_execution_stats_simulation/"
    configs['log_dir']             = configs['output_directory']+"03_execution_stats_simulation/logs/"
    configs['configs_dir']         = configs['output_directory']+"03_execution_stats_simulation/configs/"
    configs['DUMP_DIR']            = configs['output_directory']+"03_execution_stats_simulation/dumps"
    ...
    if os.path.isdir (configs['datapoints_dir']):
        for r,ds,fs in os.walk(configs['datapoints_dir']):
            RAW_FILES       = [f for f in fs if 'EXECSTATS' in f]            
