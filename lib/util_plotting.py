import sys,os,utilv4 as super_util
realp   = super_util.realp 
#######################################################################################
def myprint(s):
    sys.stdout.write(s)
    sys.stdout.flush()
...
def max_lines (pair):
    ...
    for prefix, file_path, title in pair:
        num_lines.append(sum(1 for line in open(file_path,'r')))     
    if len(num_lines)==0:
        num_lines=[0]
