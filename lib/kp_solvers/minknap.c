//  Linux: gcc -shared -Wl,-soname,minknap.so -o minknap.so -fPIC minknap.c
//  Mac:   gcc -shared -Wl,-install_name,minknap.so -o minknap.so -fPIC minknap.c
		   //comment out #include <values.h> 
		   //insert      #include <limits.h> instead
		   //comment out #include <malloc.h>, #include <stdlib.h> is enough
...
void solve (int *p, int *w, int c, int n, int *x, int *result, double *exec_time)
{
  // segmentation fault if sum(w's) < c
  int i=0, sumw=0,sumv=0;
  for (i=0;i<n;i++)
      sumw = sumw + w[i];
  if (sumw <= c){ //the knapsack can hold all objects
      for (i=0;i<n;i++){
          sumv = sumv + p[i];
          x[i]=1;
      }
    ...
    result [0] = a.zstar; // total value
    result [1] = a.cstar; // INCORRECT total weight (it may correspond to capacity (threshold) instead)
    result [2] = a.coresize;
