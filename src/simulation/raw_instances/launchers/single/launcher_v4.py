...
sys.path.insert(0, os.getenv('SIMULATION_DIRECTORYv4')) #so I can import init.py
import init
...
def launch(simulation_script, launching_script, simulation_directory,launching_directory,configs_file, qsub_simulation_arg, qsub_launching_arg, dependency_switch, log):
    ...
    os.environ['LAUNCHING_SCRIPTv4']     = launching_script
    os.environ['SIMULATION_DIRECTORYv4'] = simulation_directory
    os.environ['LAUNCHING_DIRECTORYv4']  = launching_directory
    os.environ['SIMULATION_CONFIGSv4']   = configs_file
    log.write("\nos.environ['SIMULATION_SCRIPTv4']   = "+os.getenv('SIMULATION_SCRIPTv4'))
    log.write("\nos.environ['LAUNCHING_SCRIPTv4']    = "+os.getenv('LAUNCHING_SCRIPTv4'))
    log.write("\nos.environ['SIMULATION_DIRECTORYv4']= "+os.getenv('SIMULATION_DIRECTORYv4'))
    log.write("\nos.environ['LAUNCHING_DIRECTORYv4'] = "+os.getenv('LAUNCHING_DIRECTORYv4'))
    log.write("\nos.environ['SIMULATION_CONFIGSv4']  = "+os.getenv('SIMULATION_CONFIGSv4'))
    log.write("\nPATH  = "+os.getenv('PATH'))
    log.write("\nLD_LIBRARY_PATH  = "+os.getenv('LD_LIBRARY_PATH'))
    #print("os.environ['SIMULATION_SCRIPTv4']   = "+os.getenv('SIMULATION_SCRIPTv4'))
    #print("os.environ['LAUNCHING_SCRIPTv4']    = "+os.getenv('LAUNCHING_SCRIPTv4'))
    #print("os.environ['SIMULATION_DIRECTORYv4']= "+os.getenv('SIMULATION_DIRECTORYv4'))
    #print("os.environ['LAUNCHING_DIRECTORYv4'] = "+os.getenv('LAUNCHING_DIRECTORYv4'))
...
if __name__ == "__main__":       
    log = open (os.path.join (slash(os.getenv('LAUNCHING_DIRECTORYv4')), "launcher_v4.log" ), "a") 
    if len(sys.argv) < 2:
    ...
    qsub_simulation_arg         = [sub_cmd, "-N", job_name,  "-o",  SCRATCH_DIR+"qsub_simulation_output_"+host+'_'+timestamp+".txt", "-e",  SCRATCH_DIR+"qsub_simulation_error_"+host+'_'+timestamp+".txt", "-V", slash(os.getenv('LAUNCHING_DIRECTORYv4'))+sim_script]
    qsub_launching_arg          = [sub_cmd,  "-N", job_name,  "-o",  SCRATCH_DIR+"qsub_launching_output_"+host+'_'+timestamp+".txt", "-e",  SCRATCH_DIR+"qsub_launching_error_"+host+'_'+timestamp+".txt",  "-V", slash(os.getenv('LAUNCHING_DIRECTORYv4'))+launch_script]

    simulation_script           = os.getenv('SIMULATION_SCRIPTv4') 
    launching_script            = os.getenv('LAUNCHING_SCRIPTv4') 
    simulation_directory        = os.getenv('SIMULATION_DIRECTORYv4') 
    launching_directory         = os.getenv('LAUNCHING_DIRECTORYv4')   
