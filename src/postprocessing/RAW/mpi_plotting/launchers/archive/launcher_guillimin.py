import os, sys, init, util, socket, math,time, json,subprocess
...
if __name__ == "__main__": 
    ...
    if num_workers >0: 
        ...
        launch_script  = util.slash(os.getenv('PLOTTING_LAUNCHING_SCRIPT'))
        sim_specs         = open (submission_script, 'w')
        dependency_switch ="-W" 
        
    
        if  "colosse" in host:
            print ("We are on colosse, eh! (not implemented yet**********")
            sub_cmd    = "msub"
            dependency_switch ="-l"
        ...
        qsub_launching_arg    = [sub_cmd,  "-N", job_name,  "-o", configs['output_dir']+"qsub/qsub_launching_output_" +host+'_'+configs['timestamp']+".txt", "-e",  configs['output_dir']+"qsub/qsub_launching_error_" +host+'_'+configs['timestamp']+".txt",  "-V",launch_script]
        ...
        print ("simulation job submitted: "+str(simulation_job_id))
        print ("\nplease wait ..", end='')
        time.sleep(60)
        launching_job_id    = (subprocess.Popen (qsub_launching_arg+[dependency_switch, "depend=afterany:"+simulation_job_id.split(".")[0]], stdout=subprocess.PIPE, universal_newlines=True)).stdout.read
        print ("launching job submitted: "+str(launching_job_id)+"\n")
