...
#######################################################################################
def max_instances (pair, configs, lines_per_instance):
    imposed_instances   = []
    requested_instances = None
    try:
        requested_instances = int(configs['max_instances'])
        if requested_instances <= 0:
            requested_instances = None
    except:
        requested_instances = None # read the whole file
    for p in pair:
        if not os.path.isfile(p['file_path']): # in case we're processing directories (bar3d)
            return 0
    for p in pair:
        avail_instances = int(sum([1 for line in open(p['file_path'], 'r')])/lines_per_instance)
        if requested_instances != None:
            imposed_instances.append(min(avail_instances, requested_instances))
        else:
            imposed_instances.append(avail_instances)

    imposed_instances = min(imposed_instances)
    return imposed_instances
#######################################################################################
...
def calculate_Axis_limits (pair, configs):
    max_B, max_D, num_instances = 0, 0, []
    limit = 0
    try:
        limit = int(configs['max_instances'])
        if limit<=0:
            limit = -1
    except:
    ...
    for p in pair:
        ...
        try:
            while True:
                ...
                next(df)
                instance_counter +=1
                if limit>0 and instance_counter>limit:
                    num_instances.append(instance_counter)
                    break
        except:
            num_instances.append(instance_counter)
            instance_counter=0
    ...
    try:
        if int(configs['max_instances'])>0:
            num_instances = min(int(configs['max_instances']), num_instances)
    except:
        pass
    try:
    ...
    try:
        ...
        configs['ylim'] = max_B
        pass
    return [configs['xlim'], configs['ylim'], num_instances]
#######################################################################################
...
def crunch(arguments, rank):
    ...
    processor = None
    if 'ETXcruncher' in  configs['cruncher']:
    ...
    elif configs['cruncher'] in ['coresize_cruncher', 'runtime_cruncher']:
        processor = crunchers.ExecStats_processor
    elif configs['cruncher'] in ['iSize_cruncher']:
        processor = crunchers.iSize_processor
    elif configs['cruncher'] == 'sSpace':
        processor = crunchers.sSpace_processor
    else:
        mywrite(configs['worker_log'], '\n\n\nFATAL: I dont know this cruncher: '+str(configs['cruncher'])+'\nExiting .. \n\n')
        sys.exit(1)
