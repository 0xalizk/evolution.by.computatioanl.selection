    ...
    for measure in get_MEASURES():
        ...
        for master_dump in sorted(master_dumps):        
            ...
            if measure in ['Sbin','Sdin','Sbou','Sdou']:
                ...
                configs['plotting_configs']['elev'], configs['plotting_configs']['azim'], configs['dx'], configs['dy']= 10, -1.5, 0.8, 0.1
                master.plot_next_stacked_bar3d(fig, ax, title, zlim, DICT_of_AVGs, configs, measure=measure, norm=norm, colors=get_COLORS())
                current_pos+=1
            else:
                for norm in NORMS:
                    print (norm+'-'+str(current_pos)+', ', end='')  
                    ax = fig.add_subplot (rows, cols, current_pos,  projection='3d')
                    # IF YOU WANT TO IMPOSE A GLOBAL ZLIM, UN-COMMENT THE FOLLOWING LINE (otherwise use zlim = ZLIMS[measure]):
