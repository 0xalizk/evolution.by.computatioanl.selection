...
def watch (configs, FIGURES, AXES):
    ...
    if len (AXES) == 0:
        ...
        for fig in FIGURES:
            ...
            fig.savefig(util.slash(configs['plots_dir'])+str(i).rjust(3,'0')+"."+configs['file_extension'], dpi=configs['plotting_configs']['dpi'], bbox_inches="tight")
            i+=1
...
def harvest(configs, file, AXES, FIGURES):
    ...
    plot_next_bar3d (FIGURES[file_no-1], ax, title, ZLIMS, DICT_of_AVGs, configs)
    ...
    return True
#-----------------------------------------------------------------
def plot_next_bar3d (fig, ax, title, ZLIMS, DICT_of_AVGs, configs, colors=default_colors, measure=default_measure, norm=default_norm):
...
def init_ax(ax, title, zlim, dim1, dim2, configs):
    ...
    dy = [configs['plotting_configs']['dx']]*(dim1*dim2)     #bar length
