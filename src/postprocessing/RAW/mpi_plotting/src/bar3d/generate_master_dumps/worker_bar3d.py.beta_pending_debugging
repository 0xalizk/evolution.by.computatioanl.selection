...
def setup (arguments, rank):
    ...
    return configs
#######################################################################################
def crunch (Genes,Bin,Bou,Din,Dou,Sbin,Sbou,Sdin,Sdou,Gs,Bs,Ds,Xs):
    ...
    for g,b,d,x in [(g,b,d,x) for g,b,d,x in zip(Gs,Bs,Ds,Xs)]:
        if x==1:
            if g in Genes.keys():
                Genes[g]['bin'].append(b)
                Genes[g]['din'].append(d)
            else:
                Genes[g]={'bin':[],'din':[],'bou':[],'dou':[]}
                Genes[g]['bin'].append(b)
                Genes[g]['din'].append(d)
        elif x==0:
            if g in Genes.keys():
                Genes[g]['bou'].append(b)
                Genes[g]['dou'].append(d)
            else:
                Genes[g]={'bin':[],'din':[],'bou':[],'dou':[]}
                Genes[g]['bou'].append(b)
                Genes[g]['dou'].append(d)
            ...
            sys.exit(1)
#######################################################################################
def update_aggregates (Genes,Bin,Bou,Din,Dou,avgBin,avgBou,avgDin,avgDou,ZLIMS):
    ...
    for gene in Genes.keys():
        for measure in Genes[gene].keys():#bin,din,bou,dou
            if len(Genes[gene][measure])==0:
                Genes[gene][measure] = {'count':0,'avg':0}
            else:
                Genes[gene][measure] = {'count':len(Genes[gene][measure]), 'avg':np.average(Genes[gene][measure])}
    #-----------------------------------------------------------------------
...
#######################################################################################
def zlim_and_zdata (dir_pair, max_lines, rank): 
    ...
    if len(dir_pair) > 0:
        ...
        for p,t in ALL_PTs: 
            ...
            for _ , csv, _ in current_PT:
                if rank == 1: 
                    print (("worker# "+str(rank)+": current p,t: "+str(p)+","+str(t)).ljust(40,' ')+csv.split('/')[-1])
                ...
                while step < max_lines:
                    Gs = [g      for g in next(instances).strip().split()]
                    Bs = [int(b) for b in next(instances).strip().split()]
                    Ds = [int(d) for d in next(instances).strip().split()]
                    Xs = [int(x) for x in next(instances).strip().split()]
                    Zs = [float(z) for z in next(instances).strip().split()] # exec_time, core_size                                            
                    #---------------------------------------------------------
                    crunch (Genes,Bin,Bou,Din,Dou,Sbin,Sbou,Sdin,Sdou,Gs,Bs,Ds,Xs)
                    #---------------------------------------------------------     
                    step += 5
                ...
                update_aggregates (Genes,Bin,Bou,Din,Dou,avgBin,avgBou,avgDin,avgDou,ZLIMS)                
                DICT[index][(p,t)] = {'Genes':Genes,'Bin': avgBin, 'Bou': avgBou, 'Din': avgDin, 'Dou': avgDou, 'Sbin':Sbin, 'Sbou':Sbou, 'Sdin':Sdin, 'Sdou':Sdou, 'num_instances':max_lines/5.0    }
...
def work(arguments, rank):
    ...
    if len(AXES) > 0:       
        for  pair in AXES:
            #--------------------------------------------------------------
            updated_pair  =  zlim_and_zdata (pair,configs['max_lines'], rank) # [[network_name, csv_dir_path, plot_title, zlim, {(p,t):Bin}, [...]],
            #--------------------------------------------------------------
