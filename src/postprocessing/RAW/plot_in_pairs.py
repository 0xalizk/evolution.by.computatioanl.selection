...
def green_grey_red (df, ax, configs):
    ...
    ax.bar(y_pos, categories, width,  color=colors, align='center', yerr=yerr, edgecolor='none')
...
def PLOTTER (PAIRS, configs): 
    ...
    for  pair in PAIRS:
        ...
        for tuple in pair:
            ...
            elif configs['plot']=='pearsonr_BD':
                sc, x_suff = pearsonr_BD (df, ax, configs)
            
            else:
                ax.set_ylim([0,axis_limits[0]])
                sc = percentiles (df, ax, configs)
...
if __name__ == "__main__": 
    ...
    if len (plots_root_dir) == 0:
        plots_root_dir = slash(os.getcwd()) 
    ...
    #-------------------------------------------------------------
    columns = 4 # aka no. of files per pair
    ...
    b   = Process(target=PLOTTER, args=(PAIRS[0], configs_scatter_BD,))
    c   = Process(target=PLOTTER, args=(PAIRS, configs_coresize,))
    d   = Process(target=PLOTTER, args=(PAIRS[0], configs_green_grey_red,))
    e   = Process(target=PLOTTER, args=(PAIRS[0], configs_pearsonr_BD,))
