...
def getCommandLineArg():
    try:
        ...
        print ('Usage: python3 [input-file (containing paths to AGGREGATE.csv files) ] \nExiting...')
        sys.exit(1)
    return input_file
#--------------------------------------------------------------------------------------
def get_zlim_normalized (network1, network2, measure):
    ...
    print ("\t\t"+("calculating zlimit for "+network1.split('/')[-1].split('_AGGREGATE_')[0]+" ..") .ljust(45,' '),end='')
    zlim = 0
    for df in [df1, df2]:
        zdata         = df[measure].tolist()                             
        zlim = max (zlim, max (zdata))
    print (str(math.ceil(zlim)))
    return zlim
#--------------------------------------------------------------------------------------
def getPairs (AGGREGATE_datafiles):
    PAIRS = []
    counter = 0
    ZLIM =0
    for f in range(int(len(AGGREGATE_datafiles)/2)):
        network1 = AGGREGATE_datafiles[counter].strip()
        network2 = AGGREGATE_datafiles[counter+1].strip()
        zlim     = get_zlim_normalized(network1, network2, 'coresize_avg')
        ZLIM = max(ZLIM, zlim)
...
if __name__ == "__main__": 
    ...
    for pair in PAIRS:
        ...
        #Suratanee_AGGREGATE_datapoints_V3_MINKNAP_4X_TARGET_SCRAMBLE_May-23-2016-h13m20s17.csv
        
        prefix1, prefix2 = data_file1.split('/')[-1].split('_AGGREGATE_'), data_file2.split('/')[-1].split('_AGGREGATE_') 
        suffix1, suffix2 = prefix1[1].split('_'), prefix2[1].split('_')
        title1, title2   = prefix1[0] + "["+ suffix1[4] +", "+ suffix1[5]+"]", prefix2[0] + "["+ suffix2[4] +", "+ suffix2[5]+"]"
        ...
        for  df, title in [(df1,title1), (df2, title2)]:
            #----------------------------------------------------------------------------------------------------------------------------------------------
            ax, dx, dy = init_ax (fig, len(PAIRS), 2, pos, zlim, "coresize", title)
            #----------------------------------------------------------------------------------------------------------------------------------------------              
            ...
            zdata  = df['coresize_avg'].tolist()
            
            assert len(zdata)==len(unique_Ps1)*len(unique_Ts1)
            ...
            #------------------------------------------------------------------------------------------------------------------
            ax.bar3d (Ts, Ps, z_offset, dx, dy, zdata , alpha=alpha, color="blue", edgecolor='')  #
            #------------------------------------------------------------------------------------------------------------------                
                        
            pos +=1
            
            #patches.append (mpatches.Patch(color="blue", label=current_slice_label))  
        #--------------------------------------------------LEGEND---------------------------------------------------------------------------
        #patches.reverse()
        #legend = ax.legend (handles=patches, frameon=False, borderaxespad=1.0, borderpad=1.0, fontsize= 8, markerscale=0.0, loc='upper left', bbox_to_anchor = (0.2, 0.825), title='degree range  (no. nodes)',shadow=False)                                            
        #legend.get_title().set_fontsize('9') #frame = legend.get_frame() #frame.set_facecolor('white') #frame.set_edgecolor('white')
        #------------------------------------------------------------------------------------------------------------------------------------
                                 
    print ("\nSaving figure ..")
    plt.savefig(str(input_file.split('/')[-1].split('.')[:-1])+".png")
