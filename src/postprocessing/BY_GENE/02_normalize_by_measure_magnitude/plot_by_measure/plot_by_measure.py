...
import os,sys, math, itertools, pandas as pd, numpy as np
...
def get_plots():
    plots = {
                    '1-Bin':           {'title':'Gained Benefits (knapsack value)',            'color':'YlGn',    'measure_column':'Bin_avg',        'Z_label':'Benefits (norm. by no. nodes)',     'location':(9,2)},
                    '2-Din':           {'title':'Incurred Damages (knapsack weight)',          'color':'OrRd',    'measure_column':'Din_avg',        'Z_label':'Damages (norm. by no. nodes)',      'location':(9,2)},
                    '3-Bout':          {'title':'Lost Benefits (outside knapsack)',            'color':'OrRd',    'measure_column':'Bout_avg',       'Z_label':'Benefits (norm. by no. nodes)',     'location':(9,2)},
                    '4-Dout':          {'title':'Cleansed Damages (outside knapsack)',         'color':'YlGn',    'measure_column':'Dout_avg',       'Z_label':'Damages (norm. by no. nodes)',      'location':(9,2)},
         
                    '5-Bin_over_Din':  {'title':'Knapsack Benefit-to-Damage ratio',            'color':'YlGnBu',  'measure_column':'Bin_over_Din',   'Z_label':'Benefits-in / Damages-in (norm. by no. nodes)',    'location':(9,2)},
                    '6-Bin_over_Bout': {'title':'Inside/Outside Knapsack Benefits Ratio',      'color':'PuBu',    'measure_column':'Bin_over_Bout',  'Z_label':'Benefits-in / Benefits-out (norm. by no. nodes)',  'location':(9,2)},
                    '7-Dout_over_Din': {'title':'Outside/Inside Knapsack Damage Ratio',       'color':'YlGn',    'measure_column':'Dout_over_Din',  'Z_label':'Damages-out / Damages-in (norm. by no. nodes)',    'location':(9,2)},
                    '8-Bout_over_Dout':{'title':'Outside Benefit/Outside Damage ratio',        'color':'Spectral','measure_column':'Bout_over_Dout', 'Z_label':'Benefits-out / Damages-out (norm. by no. nodes)',  'location':(9,2)},
                    '9-Bout_over_Bin': {'title':'Outside/Inside knapsack Benefit Ratio',      'color':'GnBu',    'measure_column':'Bout_over_Bin',  'Z_label':'Benefits-out / Benefits-in (norm. by no. nodes)',  'location':(9,2)}
                }
    return plots
#--------------------------------------------------------------------------------------
def get_colors():
...
def get_zlim_normalized (network1, network2, measure):
    df1                  = pd.read_csv(network1,header=0,delimiter='\t', dtype={
                                                                            ...
                                                                            'white_genes_avg':float,
                                                                            'white_genes_std':float,
                                                                            'black_genes_avg':float,
                                                                            'black_genes_std':float,
                                                                            'grey_genes_avg':float,
                                                                            'grey_genes_std':float,
                                                                            'coresize_avg':float,
                                                                            'coresize_std':float,
    ...
    unique_Ps1, unique_Ps2, unique_Ts1, unique_Ts2 = sorted(set(df1['p'])), sorted(set(df2['p'])), sorted(set(df1['t'])), sorted(set(df2['t'])) 
    #print ("\nunique_Ps1 "+str((unique_Ps1))+"\nunique_Ps2 "+str((unique_Ps2))+"\nunique_Ts1 "+str((unique_Ts1))+"\nunique_Ts2 "+str((unique_Ts2)))
    assert len(unique_Ps1) == len(unique_Ps2) and len(unique_Ts1) == len(unique_Ts2)
    ...
    for df, bins in [(df1, bins1), (df2, bins2)]:
        ...
        for deg_range in bins:
            ...
            if (i<(len(bins)-1)):                  
                current_slice       = pd.DataFrame (df[         (df[plots[measure]['measure_column']] >= bins[i])   &    (df[plots[measure]['measure_column']] < bins[i+1])    ])                        
            else:                 
                current_slice       = pd.DataFrame (df[         (df[plots[measure]['measure_column']] >= bins[i])      ])   
                      
            divider       = float (len (current_slice.groupby(['Gene'])))               
            current_slice = current_slice.groupby(['p', 't']).sum().reset_index()               

            for PT_pair in itertools.product(unique_Ps1, unique_Ts1):
                if len(current_slice[         (current_slice['p'] == PT_pair[0])   &    (current_slice['t'] == PT_pair[1])    ]) == 0 :
                    current_slice.loc[len(current_slice)] = [PT_pair[0], PT_pair[1]]+[0]*(len(current_slice.columns)-2)
                                                
            current_slice = current_slice.sort_values(by=['p','t'])
...
if __name__ == "__main__": 
    input_file, plots, colors = getCommandLineArg(), get_plots() , get_colors()
    dir_name = input_file.split('/')[-1].strip().split('.')[0].strip()
    plots_dir =  os.path.join(os.getcwd(), dir_name)
    ...
    for measure in sorted(plots.keys()):
        ...
        for pair in getPairs (datapoints_files, measure):
            ...
            print ("\n\t\tplotting "+str(data_file1.split('/')[-1])+"\n\t\tplotting "+str(data_file2.split('/')[-1]))
            ...
            for  df, bins, col, title in [(df1,bins1,0, title1), (df2,bins2,1, title2)]:
                ...
                for magnitute in bins:
                    ...
                    divider = float (len (current_slice.groupby(['Gene'])))
                    current_slice = current_slice.groupby(['p', 't']).sum().reset_index()  
                    print ("\t\t"+str(len(current_slice)),end='')
                    #current_slice.to_csv(measure+'_before.csv')
                    #pd.isnull(df2['one'])
                    for PT_pair in itertools.product(unique_Ps1, unique_Ts1):
                        if     len(current_slice[         (current_slice['p'] == PT_pair[0])   &    (current_slice['t'] == PT_pair[1])    ]) == 0 :
                            current_slice.loc[len(current_slice)] = [PT_pair[0], PT_pair[1]]+[0]*(len(current_slice.columns)-2)
                    ...
                    print (" >> "+str(len(current_slice)), end='')
                    #current_slice.to_csv(measure+'_after.csv')
                ...
                legend = ax.legend (handles=patches, frameon=False, borderaxespad=1.0, borderpad=1.0, fontsize= 8, markerscale=0.0, loc='upper left',
                                    bbox_to_anchor = (0.2, 0.825), title=plots[measure]['measure_column']+' (no. nodes)',shadow=False)                  
