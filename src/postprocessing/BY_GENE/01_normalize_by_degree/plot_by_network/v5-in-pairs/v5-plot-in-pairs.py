...
plots = {
         ...
         '7-Dout_over_Din': {'title':'Outside/Inside Knapsack Damages Ratio',       'color':'YlGn',    'measure_column':'Dout_over_Din',  'Z_label':'Damages-out / Damages-in',    'location':(9,2)},
         '8-Bout_over_Dout':{'title':'Outside/Outside knapsack  Damages Ratio',     'color':'Spectral','measure_column':'Bout_over_Dout', 'Z_label':'Benefits-out / Damages-out',  'location':(9,2)},
         '9-Bout_over_Bin': {'title':'Outside/Outside knapsack Benefits Ratio',     'color':'GnBu',    'measure_column':'Bout_over_Bin',  'Z_label':'Benefits-out / Benefits-in',  'location':(9,2)}
        }
...
BY_GENE_datafiles    = (open (str(sys.argv[1]),'r')).readlines()

PAIRS = []
counter = 0

for f in range(int(len(BY_GENE_datafiles)/2)):
    PAIRS.append([  BY_GENE_datafiles[counter], BY_GENE_datafiles[counter+1]  ])
    counter +=2
...
for pair in PAIRS:
    ...
    print ("processing "+str(data_file2.split('/')[-1]))
    ...
    unique_degrees1 = sorted(set (df1['degree']))    
    unique_degrees2 = sorted(set (df2['degree']))  
    bins1           = sorted(list(
                          set(
                              [round(d) for d in np.histogram(unique_degrees1,bins=min(6, len(unique_degrees1)))[1]][:-1]) ))   #= [0,1, 2, 3, 4, 5, df['degree'].max()+1] 
    
    bins2           = sorted(list(
                          set(
                              [round(d) for d in np.histogram(unique_degrees2,bins=min(6, len(unique_degrees2)))[1]][:-1]) ))   #= [0,1, 2, 3, 4, 5, df['degree'].max()+1] 
 
    print ("unique degrees1: "+str(set(df1['degree'])))
    print ("bins1 "+str(bins1))
    print ("unique degrees2: "+str(set(df2['degree'])))
    print ("bins2 "+str(bins2))
    ...
    for measure in sorted(plots.keys()):
        ...
        for  df, bins, col in [(df1,bins1,0), (df2,bins2,1)]:
            ...
            xticks = [x+offx for x in xticks]
            yticks = [y+offy for y in yticks]
            ...
            ax.set_xticklabels(xlabels,rotation=-90, verticalalignment='baseline', horizontalalignment='center')    
            ax.set_yticklabels(ylabels,rotation=-90, verticalalignment='baseline', horizontalalignment='center')
            ...
            for deg_range in bins:
                ...
                if (i<(len(bins)-1)):
                    ...
                    current_range = (str(int(bins[i])) + "-"+str(int(bins[i+1]))).ljust(15,' ') + "("+str(current_slice['degree'].count())+")"
                else:
                    current_slice = pd.DataFrame (df[      (df['degree'] >= bins[i])      ])
                    current_range = (">="+str(int(bins[i]))).ljust(15,' ') +"("+str(current_slice['degree'].count())  + ")"
                ...
                patches.append (mpatches.Patch(color=colors[plots[measure]['color']][i], label=current_range))
