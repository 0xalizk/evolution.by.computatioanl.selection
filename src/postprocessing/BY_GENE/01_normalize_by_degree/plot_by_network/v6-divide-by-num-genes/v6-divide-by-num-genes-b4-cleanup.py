...
def get_plots():
    plots = {
                    '1-Bin':           {'title':'Gained Benefits (knapsack value)',            'color':'YlGn',    'measure_column':'Bin_avg',        'Z_label':'Benefits (no. edges)',     'location':(9,2)},
                    '2-Din':           {'title':'Incurred Damages (knapsack weight)',          'color':'OrRd',    'measure_column':'Din_avg',        'Z_label':'Damages (no. edges)',      'location':(9,2)},
                    '3-Bout':          {'title':'Lost Benefits (outside knapsack)',            'color':'OrRd',    'measure_column':'Bout_avg',       'Z_label':'Benefits (no. edges)',     'location':(9,2)},
                    '4-Dout':          {'title':'Cleansed Damages (outside knapsack)',         'color':'YlGn',    'measure_column':'Dout_avg',       'Z_label':'Damages (no. edges)',      'location':(9,2)},
         
                    '5-Bin_over_Din':  {'title':'Knapsack Benefit-to-Damage ratio',            'color':'YlGnBu',  'measure_column':'Bin_over_Din',   'Z_label':'Benefits-in / Damages-in',    'location':(9,2)},
                    '6-Bin_over_Bout': {'title':'Inside/Outside Knapsack Benefits Ratio',      'color':'PuBu',    'measure_column':'Bin_over_Bout',  'Z_label':'Benefits-in / Benefits-out',  'location':(9,2)},
                    '7-Dout_over_Din': {'title':'Outside/Inside Knapsack Damage Ratio',       'color':'YlGn',    'measure_column':'Dout_over_Din',  'Z_label':'Damages-out / Damages-in',    'location':(9,2)},
                    '8-Bout_over_Dout':{'title':'Outside Benefit/Outside Damage ratio',        'color':'Spectral','measure_column':'Bout_over_Dout', 'Z_label':'Benefits-out / Damages-out',  'location':(9,2)},
                    '9-Bout_over_Bin': {'title':'Outside/Inside knapsack Benefit Ratio',      'color':'GnBu',    'measure_column':'Bout_over_Bin',  'Z_label':'Benefits-out / Benefits-in',  'location':(9,2)}
                }
    return plots
#--------------------------------------------------------------------------------------
def get_colors():
...
def getCommandLineArg():
    ...
    return input_file
#--------------------------------------------------------------------------------------
def getZlim_normalized (network1, network2):
    ...
    plots                = get_plots()
    #columns              = [m['measure_column'] for m in plots.keys()]
    return {'1-Bin':40 ,'2-Din':40 ,'3-Bout':40 ,'4-Dout': 40,'5-Bin_over_Din': 40,'6-Bin_over_Bout': 40,'7-Dout_over_Din': 40,'8-Bout_over_Dout':40,'9-Bout_over_Bin':40}
#--------------------------------------------------------------------------------------
def getPairs (BY_GENE_datafiles):
    ...
    for f in range(int(len(BY_GENE_datafiles)/2)):
        ...
        zlim     = getZlim_normalized(network1, network2)
...
if __name__ == "__main__": 
    ...
    for pair in getPairs (open (input_file).readlines()):

        data_file1 = pair[0].strip()
        data_file2 = pair[1].strip()
        zlimit     = pair[2]   # a dictionary {measure:zlim}
        print (">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ")
        print ("processing "+str(data_file1.split('/')[-1])+"\nprocessing "+str(data_file2.split('/')[-1]))
        ...
        bins1, bins2 = [1.0, 6.0, 11.0, 16] , [1.0, 6.0, 11.0, 16]
    
        fig = plt.figure(figsize=(30, 70))
        fig.subplots_adjust(hspace = .01, wspace=.01)
        grouped1 = df1.groupby(['p', 't']).sum().reset_index()
        grouped2 = df2.groupby(['p', 't']).sum().reset_index()
        #zlim1    = max (grouped1['Bin_avg'].max(), grouped1['Din_avg'].max(), grouped1['Bout_avg'].max(), grouped1['Dout_avg'].max())
        #zlim2    = max (grouped2['Bin_avg'].max(), grouped2['Din_avg'].max(), grouped2['Bout_avg'].max(), grouped2['Dout_avg'].max())
        
    
        pos=1
        for measure in ['1-Bin']: #sorted(plots.keys()):
            print ("\t"+measure)
            zlim = zlimit[measure]
            for  df, bins, col in [(df1,bins1,0), (df2,bins2,1)]:
            
                ax = fig.add_subplot(plots[measure]['location'][0], plots[measure]['location'][1], pos+col , projection='3d')
            
                ax.set_title(plots[measure]['title'])
                dx = [.2]*(100)	 #bar_width
                dy = [.2]*(100)  #bar length
                ...
                ax.set_zticks (    range(5,zlim+1,5)         )
                ...
                ax.set_zticklabels ([str(zl) for zl in range(5,zlim+1,5)])
                ...
                for deg_range in bins:
                    ...
                    if (i<(len(bins)-1)):
                        ...
                        current_slice_label = (str(int(bins[i])) + "-"+str(int(bins[i+1]))).ljust(15,' ')  + "("+str(len(current_slice.groupby(['Gene'])))+")"     
                    else:
                        current_slice = pd.DataFrame (df[      (df['degree'] >= bins[i])      ])
                        current_slice_label = (">="+str(int(bins[i]))).ljust(15,' ') + "("+str(len(current_slice.groupby(['Gene'])))+")"
                                            
                    divider = len (current_slice.groupby(['Gene']))
                    print ("\t\t\tdivider: "+str(divider))
                    #current_slice.to_csv (current_slice_label.replace('>=','greater_than')+'.csv', sep='\t')
                    ...
                    if len(zdata)>0:
                        ...
                        Ts, Ps        = np.meshgrid ([x+.75 for x in xticks], [x-.5 for x in xticks] )               
                ...
                legend = ax.legend (handles=patches,
                                  ...
                                  title='degree range      no. contributing nodes',
                                  shadow=False
                                  ) 
            ...
            pos +=2                          
        plt.savefig(plots_dir+"/"+(data_file1.split('/')[-1])[0:-4]+".png")