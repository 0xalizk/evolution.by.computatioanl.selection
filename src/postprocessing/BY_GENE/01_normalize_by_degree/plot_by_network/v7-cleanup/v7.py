...
def getCommandLineArg():
    ...
    return input_file
#--------------------------------------------------------------------------------------
def get_zlim_normalized (network1, network2):
    ...
    plots                = get_plots()
    ZLIMS = {}
    print ("processing "+network1.split('/')[-1])
...
def getPairs (BY_GENE_datafiles):
    ...
    for f in range(int(len(BY_GENE_datafiles)/2)):
        ...
        zlim     = get_zlim_normalized(network1, network2)
...
if __name__ == "__main__": 
    ...
    for pair in getPairs (open (input_file).readlines()):
        data_file1, data_file2, Z_LIMITS = pair[0].strip() ,  pair[1].strip()  ,  pair[2]
        ...
        for measure in sorted(plots.keys()):
            ...
            for  df, bins, col in [(df1,bins1,0), (df2,bins2,1)]:
                #----------------------------------------------------------------------------------------------------------------------------------------------
                ax, dx, dy = init_ax (fig, plots[measure]['location'][0], plots[measure]['location'][1], pos+col, Z_LIMITS[measure], plots[measure]['Z_label'])
                #----------------------------------------------------------------------------------------------------------------------------------------------              
