...
def get_plots():
    plots = {
                    'BD_correlation':           {'title':'Benefit-Damage correlation',            'color':'YlGn',            'Z_label':'',     'location':(3,2)},    
...
def getPairs (BY_GENE_datafiles):
    ...
    return PAIRS
#--------------------------------------------------------------------------------------        
def init_ax(fig, dim1,dim2,loc, title):
    ax = fig.add_subplot(dim1,dim2,loc)
...
if __name__ == "__main__": 
    ...
    for measure in sorted(plots.keys()):
        ...
        fig.suptitle('BD Correcation at pressure = '+str(p)+' and tolerance = '+str(t), fontsize=16)
        pos=1
        zlim = 100
        ...
        for pair in PAIRS:
            ...
            df1                  = pd.read_csv(data_file1,header=0,delimiter='\t', dtype={
                                                                            ...
                                                                            'in_white_genes':float,
                                                                            'in_black_genes':float,
                                                                            'in_grey_genes':float
                                                                            })
            ...
            for  df, col, title in [(df1,0, title1), (df2,1, title2)]:
                ...
                slice       = pd.DataFrame (df[         (df['p'] == p)   &    (df['t'] == t)    ])

                Bs =  [bi + bo for bi, bo in  zip (slice['Bin_avg'] , slice['Bout_avg'])]
                Ds =  [di + do for di, do in  zip (slice['Din_avg'] , slice['Dout_avg'])]
                ...
                plt.tick_params(axis='x', which='both', left='off', right='off', bottom='off', top='off',  labelbottom='off', labeltop='off') # both major and minor ticks
                plt.tick_params(axis='y', which='both', bottom='off', top='off', left='off', right='off',  labelleft='off', labelright='off') # both major and minor ticks
        ...
        plt.savefig(plots_dir+"/"+measure+"_p"+str(p)+"_t"+str(t)+".png")
