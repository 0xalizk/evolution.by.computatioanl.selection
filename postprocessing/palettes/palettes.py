...
plots = {
         'Bin':           {'color':'flatui', 'measure_column':'Bin_avg',        'Z_label':'Benefits (knapsack value)', 'location':111},
...
zlim = 4000#max (grouped['Bin_avg'].max(), grouped['Din_avg'].max(), grouped['Bout_avg'].max(), grouped['Dout_avg'].max())
...
for color in colors.keys():
    ...
    for deg_range in labels:
        ...
        Ts, Ps = np.meshgrid ([x-.25 for x in xticks], [x-.25 for x in xticks] )
        ...
        ax.bar3d (Ts, Ps, z_offset, dx, dy, zdata ,alpha=alpha,color=colors[color][i], edgecolor='')
...
#    print (s)
    ...
    plt.savefig((data_file.split('/')[-1])[0:-4]+"-"+color+".png", dpi=100)
#plt.show()
