...
def getPairs (BY_GENE_datafiles):
    ...
    return PAIRS
#--------------------------------------------------------------------------------------        
def init_ax(fig, dim1,dim2,loc,zlim,zlabel):
...
if __name__ == "__main__": 
    ...
    for pair in getPairs (open (input_file).readlines()):
        ...
        print (">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\nprocessing "+str(data_file1.split('/')[-1])+"\nprocessing "+str(data_file2.split('/')[-1]))
        ...
        for measure in ['1-Bin']: #sorted(plots.keys()):
            ...
            for  df, bins, col in [(df1,bins1,0), (df2,bins2,1)]:
                ...
                ax = init_ax (fig, plots[measure]['location'][0], plots[measure]['location'][1], pos+col, zlim, plots[measure]['Z_label'])
                ...
                for deg_range in bins:
                    ...
                    if len(zdata)>0:
                        ...
                        Ts, Ps        = np.meshgrid ([x+.75 for x in get_ticks()[0]], [x-.5 for x in get_ticks()[0]] )               
