...
plots = {
         'Bin':           {'title':'Gained Benefits (knapsack value)',            'color':'YlGn',    'measure_column':'Bin_avg',        'Z_label':'Benefits (no. edges)',     'location':521},
         'Din':           {'title':'Incurred Damages (knapsack weight)',          'color':'OrRd',    'measure_column':'Din_avg',        'Z_label':'Damages (no. edges)',      'location':522},
         'Bout':          {'title':'Lost Benefits (outside knapsack)',            'color':'OrRd',    'measure_column':'Bout_avg',       'Z_label':'Benefits (no. edges)',     'location':523},
         'Dout':          {'title':'Cleansed Damages (outside knapsack)',         'color':'YlGn',    'measure_column':'Dout_avg',       'Z_label':'Damages (no. edges)',      'location':524},
         
         'Bin_over_Din':  {'title':'Knapsack Benefit-to-Damage ratio',            'color':'YlGnBu',  'measure_column':'Bin_over_Din',   'Z_label':'Benefits-in / Damages-in',    'location':525},
         'Bin_over_Bout': {'title':'Inside/Outside Knapsack Benefits Ratio',      'color':'PuBu',    'measure_column':'Bin_over_Bout',  'Z_label':'Benefits-in / Benefits-out',  'location':526},
         'Dout_over_Din': {'title':'Outside/Inside Knapsack Damages Ratio',       'color':'YlGn',    'measure_column':'Dout_over_Din',  'Z_label':'Damages-out / Damages-in',    'location':527},
         'Bout_over_Dout':{'title':'Outside/Outside knapsack  Damages Ratio',     'color':'Spectral','measure_column':'Bout_over_Dout', 'Z_label':'Benefits-out / Damages-out',  'location':528},
         'Bout_over_Bin': {'title':'Outside/Outside knapsack Benefits Ratio',     'color':'GnBu',    'measure_column':'Bout_over_Bin',  'Z_label':'Benefits-out / Benefits-in',  'location':529}
        }
...
for data_file in  BY_GENE_datafiles:
    data_file = data_file.strip()
    print ("processing "+str(data_file.split('/')[-1]))
    df                  = pd.read_csv(data_file,header=0,delimiter='\t', dtype={
                                                                            'Gene':str,
                                                                            'p':float,
                                                                            't':float,
                                                                            'degree':float,
    ...
    try:
        ...
        assert len (df['p'])       ==     (len(set (df['p'].tolist()))  * len(set (df['t'].tolist()))   * len(set (df['Gene'].tolist()))  )
        assert len (df['t'])       ==     (len(set (df['p'].tolist()))  * len(set (df['t'].tolist()))   * len(set (df['Gene'].tolist()))  )
        assert len (df['Gene'])    ==     (len(set (df['p'].tolist()))  * len(set (df['t'].tolist()))   * len(set (df['Gene'].tolist()))  )
    except:
        print ("FAILED ...")
        continue
    duplicates  = len(df.duplicated (subset=['p','t', 'Gene'])) - len(df.drop_duplicates (subset=['p','t', 'Gene']))
    print ("duplicates: "+ str(duplicates))


    unique_degrees = sorted(set (df['degree']))    
    bins           = sorted(list(
                          set(
                              [round(d) for d in np.histogram(unique_degrees,bins=min(6, len(unique_degrees)))[1]][:-1]) ))   #= [0,1, 2, 3, 4, 5, df['degree'].max()+1] 
    print ("unique degrees: "+str(set(df['degree'])))
    print ("bins "+str(bins))
    ...
    for measure in plots.keys():
        ...
        for deg_range in bins:
            if (i<(len(bins)-1)):
                #print ( "["+str(int(bins[i])) + ","+str(int(bins[i+1])) +")" )
    ...
    print("\t\tby-degree slices:")
    for r,s in zip(bins, sizes):
        print ("\t\t\t"+str(r)+"\t"+str(s))

    plt.savefig(plots_dir+"/"+(data_file.split('/')[-1])[0:-4]+".png")
